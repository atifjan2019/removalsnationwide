...
const components: Components = {
  ...
  ol: ({ children }) => (
    <ol className="mt-4 list-decimal space-y-2 pl-6 marker:font-semibold marker:text-brand-orange">
      {children}
    </ol>
  ),
  li: ({ children }) => (
    <li className="text-base leading-relaxed text-brand-charcoal/85 before:absolute before:left-0 before:text-brand-orange [ol>&]:before:content-none [ul>&]:before:content-['•']">
      {children}
    </li>
  ),
  blockquote: ({ children }) => (
    <blockquote className="mt-6 rounded-r-lg border-l-4 border-brand-orange bg-brand-grey p-5 italic text-brand-charcoal/85">
      {children}
    </blockquote>
  ),
  a: ({ href, children }) => (
